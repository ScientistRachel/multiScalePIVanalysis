...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% function correlatePIV_r(piv_values,t_scale,r_scale,sub_mean,lastframe,PIVtopixel,max_dist,savedir)
function correlatePIV_r(piv_values,x,y,t_scale,r_scale,sub_mean,lastframe,max_dist,savedir,title_text)
...
%     end
    ...
    for kk = 1:c
        slice = piv_values(:,:,kk);
        corr_temp = slice(subs1).*slice(subs2);
        term1 = nansum(nansum(slice(subs1).^2));
        term2 = nansum(nansum(slice(subs2).^2));
...
% Average over time
Cr_mean = mean(Cr,2);
% r_to_plot = linspace(bin_mids(1),bin_mids(end),200);
% options = optimset('Display','none');
% 
% % Fit average to exponential
% if sub_mean
%     error('not yet')
% else
%     p = polyfit(bin_mids,log(Cr_mean'),1);
%     x01 = [real(-1/p(1)) min(Cr_mean)];
%     
%     p = lsqnonlin(@expweightO,x01,[0 -Inf],[],options,bin_mids,Cr_mean');
%     fval1 = (1-p(2))*exp(-r_to_plot/p(1))+p(2);
%     
%     p2 = lsqnonlin(@exp2weightO,[p p p(2)],[],[],options,bin_mids,Cr_mean');
%     fval2 = p2(1)*exp(-r_to_plot/p2(2)) + p2(3)*exp(-r_to_plot/p2(4)) + p2(5);
% end

% Plot time average
figure(1)
% plot(r_to_plot,fval1,'-k','LineWidth',2)
% hold on
% plot(r_to_plot,fval2,'-','LineWidth',2)
plot(bin_mids,Cr_mean,'s','LineWidth',2)
ylim(ylim_to_use)
xlabel('\Deltar (\mum)','FontSize',16)
ylabel('C(\Deltar)','FontSize',16)
% legend(['Exponential Fit (r_c = ' num2str(p(1),'%0.2f') '\mum'],'Correlation Data')
set(gca,'FontSize',16)
...
if exist('savedir','var') && ~isempty(savedir)
    savename = [savedir ' Spatial Correlation'];
...
% Save if there is a file name given
if exist('savedir','var') && ~isempty(savedir)
    savename = [savedir ' Spatial Correlation over Time'];
    saveas(gcf,[savename '.fig'],'fig')
    saveas(gcf,[savename '.png'],'png')
end

save([savedir '.mat'],'r_bin_size','time_bin','Cr','N','Cr_mean',...
    'time_vec','n','max_dist','bin_mids')


% %%%%%%%%%% Exponential Fitting Functions
% 
%     % Fit to a single exponential - NO OFFSET - Force through 1
%     function diff = expweight(p,X,Y)
%         % Y = exp(-X/p)
%         fitY = exp(-X/p(1));
%         raw_diff = fitY - Y;
%         diff = raw_diff.*sqrt(Cw);
%     end
% 
%     % Fit to a single exponential with offset - Force through 1
%     function diff = expweightO(p,X,Y)
%         % Y = p(2) + (1 - p(2))*exp(-X/p(1))
%         fitY = (1-p(2))*exp(-X/p(1))+p(2);
% %         raw_diff = fitY - Y;
% %         diff = raw_diff.*sqrt(Cw);
%         diff = fitY - Y;
%     end
% 
%     % Fit to a double exponential - NO OFFSET
%     function diff = exp2weight(p,X,Y)
%         % Y = p(1)exp(-X/p(2)) + p(3)exp(-X/p(3));
%         fitY = p(1)*exp(-X/p(2)) + p(3)*exp(-X/p(4));
%         raw_diff = fitY - Y;
%         diff = raw_diff.*sqrt(Cw);
%     end
% 
%     % Fit to a double exponential with offset
%     function diff = exp2weightO(p,X,Y)
