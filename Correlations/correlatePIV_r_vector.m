...
function correlatePIV_r_vector(u,v,x,y,t_scale,r_scale,sub_mean,lastframe,max_dist,savedir,title_text)
...
%     end
    ...
    for kk = 1:c
        ...
        u_corr_temp = u_slice(subs1).*u_slice(subs2);
        v_corr_temp = v_slice(subs1).*v_slice(subs2);
        corr_temp = u_corr_temp + v_corr_temp;
        
        u_term1 = nansum(nansum(u_slice(subs1).^2));
        u_term2 = nansum(nansum(u_slice(subs2).^2));
        v_term1 = nansum(nansum(v_slice(subs1).^2));
        v_term2 = nansum(nansum(v_slice(subs2).^2));
