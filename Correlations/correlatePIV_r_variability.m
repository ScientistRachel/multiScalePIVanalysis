...
function correlatePIV_r_variability(piv_values,x,y,t_scale,sub_mean,lastframe,max_dist,savedir,title_text)
...
N = Cr;
Cr_spatial = cell(length(r_bins),1);
Cr_m = Cr; Cr_std = Cr;
...
%         term2 = nansum(nansum(slice2.^2));
        term1 = slice1.^2;
        term1 = sum(term1(~isnan(term1)));
        term2 = slice2.^2;
        term2 = sum(term2(~isnan(term2)));
%         Cr(jj,kk) = nansum(nansum(corr_temp))./(sqrt(term1.*term2));
        Cr(jj,kk) = sum(corr_temp(~isnan(corr_temp)))./(sqrt(term1.*term2));
        N(jj,kk) = sum(sum(~isnan(corr_temp)));
        Cr_spatial{jj}(:,kk) = numel(find(~isnan(corr_temp))).*corr_temp./(sqrt(term1.*term2));
        Cr_m(jj,kk) = numel(find(~isnan(corr_temp))).*mean(corr_temp(~isnan(corr_temp)))./(sqrt(term1.*term2));
        Cr_std(jj,kk) = numel(find(~isnan(corr_temp))).*std(corr_temp(~isnan(corr_temp)))./(sqrt(term1.*term2));
    end   
    
end

tstar1 = tinv(1-0.05/2,N(:,1)-1);
tstarEnd = tinv(1-0.05/2,N(:,end)-1);
% Plot time average
figure(1)
errorbar(bin_mids,Cr_m(:,1),Cr_std(:,1)./sqrt(N(:,1)).*tstar1,'s','LineWidth',2)
hold on
errorbar(bin_mids,Cr_m(:,end),Cr_std(:,end)./sqrt(N(:,end)).*tstarEnd,'s','LineWidth',2)
hold off
% ylim(ylim_to_use)
...
if exist('savedir','var') && ~isempty(savedir)
    savename = [savedir ' Spatial Correlation - NaNnorm - Spatial Variation'];
...
save([savedir 'NaNnorm.mat'],'r_bin_size','Cr','N','Cr_spatial',...
    'Cr_m','Cr_std','max_dist','bin_mids','-v7.3')
