...
function [mean_ftle, SEM_ftle, per_pos, firstframe, lastframe] = ftle_analyticsMultiC(directory, imname, ftleLRC, r_scale, t_scale, firstframe, lastframe)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% INPUTS %%%
...
% 2016/08/01 RML use either original or MultiC position lists
% 2016/10/04 RML correctly organize the FTLE files for either original or
%            MultiC position lists
%            Also, if 'bad r' or other edge detection errors, use warning 
%            and NaN values instead of error message
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
if exist([directory 'Analytics Data\' imname '_dot_RvsT.mat'],'file')
    ...
    % All outputs in um
    [X,Y,R,x_pos,y_pos] = do_edgedat_to_circleMultiC(DotE{1}, DotE{2}, param.pos_list, directory, imname,r_scale);
    clear DotE edgelist
...
for kk = 1:size(ftles,3)
    ...
    test = r_scaled(:,:,kk) > param.edge_cut;
    if sum(test(:)) == 0
        warning(['bad r - values NaN on frame ' num2str(kk)])
        slice2 = NaN;
    else
        slice2 = slice(r_scaled(:,:,kk) > param.edge_cut);
        N_t_edge(kk) = numel(find(~isnan(slice2)));
        if N_t_edge(kk) == 0
            warning(['no values left - values NaN on frame ' num2str(kk)])
            mean_ftle_t_edge(kk) = NaN;
        else
            mean_ftle_t_edge(kk) = mean(slice2(~isnan(slice2)));
            if isnan(mean_ftle_t_edge(kk))
                warning(['bad mean - values NaN on frame ' num2str(kk)])
            end
        end
    end
