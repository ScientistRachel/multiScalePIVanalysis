function run_ftle_time_histPairedTif(directory,r_scale, t_scale, firstframe, lastframe,over_write)
...
for jj = 1:size(names_list,1)
    ...
    if ~exist([directory 'FTLE Analytics\' imname '_FTLE_edge.mat'],'file') || over_write

        disp(['    FTLE histograms running on ' imname ' (' num2str(jj) ' of ' num2str(size(names_list,1)) ')'])
        ...
        ftle_time_hist_PairedTif(directory, imnames, ftleLRC, r_scale, t_scale, firstframe, lastframe)
        close all
