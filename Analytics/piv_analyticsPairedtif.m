...
function [ang_dev, mean_speed, SEM_speed, deltaR, firstframe, lastframe] = piv_analyticsPairedtif(directory, imnames, r_scale, t_scale, firstframe, lastframe,pos_list)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%% INPUTS %%%
...
% 2021/01/04 RML    Added angular deviation to r/R metrics
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
param.rose_limit = .3;%.25; % Set axis limit for rose plot
...
if param.pos_list == 2
    ...
    load([directory 'Analytics Data\' imnames{1} '_' imnames{2} '_dot_RvsT.mat'],'X','Y','R','x_pos','y_pos')
...
title_text = ['Mean Speed: ' num2str(mean(speed_slice(~isnan(speed_slice))),'%0.03f') ' ' char(181) 'm/min'];
bar_text = [char(181) 'm/min'];
savename = [directory 'Analytics Figures\' imnames{1} '_' imnames{2} 'speed_kymo'];

plot_kymo(speed_slice,r(:,:,1),t_scale,param.r_step,param.numel_limit,1,lastframe-firstframe+1,param.speed_limits,title_text,savename,bar_text,[imnames{1} '_' imnames{2}])
...
bar_text = 'cos(\theta)';
savename = [directory 'Analytics Figures\' imnames{1} '_' imnames{2} 'cos_kymo'];

plot_kymo(cos(angles),r(:,:,1),t_scale,param.r_step,param.numel_limit,1,lastframe-firstframe+1,param.cos_limits,title_text,savename,bar_text,[imnames{1} '_' imnames{2}])

%%%%%%%%%%%%%%%%%%%%%%%%%%%% Rose Plots %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
angles = angles(:,:,firstframe:lastframe);
% Rose plot is normalized by the total count
% All vectors
savename = [directory 'Analytics Figures\' imnames{1} '_' imnames{2} 'rose_total'];
title_text = ['Total Angular Deviation: ' num2str(ang_dev(1),'%0.03f')];
rose_normal2(angles,param.angle_bins,param.rose_limit,savename,[],title_text,[imnames{1} '_' imnames{2}])

% Edge vectors only
ang_dev(2) = calc_ang_dev(angles(r_scaled > param.edge_cut));
title_text = ['Edge Angular Deviation: ' num2str(ang_dev(2),'%0.03f')];
savename = [directory 'Analytics Figures\' imnames{1} '_' imnames{2} 'rose_edge'];
rose_normal2(angles(r_scaled > param.edge_cut),param.angle_bins,param.rose_limit,savename,[],title_text,[imnames{1} '_' imnames{2}])

% Center vectors only
ang_dev(3) = calc_ang_dev(angles(r_scaled < param.center_cut));
title_text = ['Center Angular Deviation: ' num2str(ang_dev(3),'%0.03f')];
savename = [directory 'Analytics Figures\' imnames{1} '_' imnames{2} 'rose_center'];
rose_normal2(angles(r_scaled < param.center_cut),param.angle_bins,param.rose_limit,savename,[],title_text,[imnames{1} '_' imnames{2}])
...
title([imnames{1} '_' imnames{2}],'FontSize',16,'Interpreter','none')
set(gca,'FontSize',16)
xlim([time_vec(1) time_vec(end)])

savename = [directory 'Analytics Figures\' imnames{1} '_' imnames{2} 'size'];
...
for j = 1:(length(bins) - 1)
    ...
    if N(j) > param.size_limit
        ...
        % Angular deviation
        angdev_r(j) = calc_ang_dev(ang_now);
...
% Save components
save([directory 'Analytics Data\' imnames{1} '_' imnames{2} '_vel_comp.mat'],'speed_mean','speed_std',...
    'v_r_mean','v_r_std','v_th_mean','v_th_std','v_th_mean_abs','v_th_std_abs','angdev_r',...
...
title([imnames{1} '_' imnames{2} 10 'Speed Components'],'FontSize',16,'Interpreter','none')
set(gca,'FontSize',16)
xlim([0 max(bins)+param.R_bin_size])
% Save figure
savename = [directory 'Analytics Figures\' imnames{1} '_' imnames{2} 'Radial Speed'];
saveas(gcf,[savename '.fig'],'fig')
saveas(gcf,[savename '.tif'],'tif')
...
title([imnames{1} '_' imnames{2} 10 'Velocity Components'],'FontSize',16,'Interpreter','none')
set(gca,'FontSize',16)
xlim([0 max(bins)+param.R_bin_size])
% Save figure
savename = [directory 'Analytics Figures\' imnames{1} '_' imnames{2} 'Radial Velocity'];
saveas(gcf,[savename '.fig'],'fig')
saveas(gcf,[savename '.tif'],'tif')

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% Save Data %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
save([directory 'Analytics Data\' imnames{1} '_' imnames{2} 'Analytics.mat'],'ang_dev',...
