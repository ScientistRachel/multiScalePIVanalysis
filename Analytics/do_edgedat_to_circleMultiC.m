...
function [X,Y,R,x_pos,y_pos] = do_edgedat_to_circleMultiC(DotL, DotR, pos_list, directory, imname,r_scale)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%% INPUTS %%%%%%
...
base_length = length(imname)-2;
piece = cell(numel(list),1);
for kk = 1:numel(list)
    if strcmp(list(kk).name(end-19:end),'dot_piv_filtered.mat')
        piece{kk} = list(kk).name(base_length:end-20); %Edge pieces
    else
        piece{kk} = list(kk).name(base_length:end-11); %Center pieces
    end
end
...
%     poslist_file = [directory 'PosList.csv'];
    ...
    for kk = 1:length(piece)
        found = 0;
        k = 10; %there is junk at the top of the excel file, avoid by starting at k = 10;
...
save([directory 'Analytics Data\' imname '_dot_RvsT.mat'],'X','Y','R','x_pos','y_pos')
