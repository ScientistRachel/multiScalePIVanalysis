function basic_analytics(directory,r_scale, t_scale, firstframe, user_lastframe,data_type,over_write)

% Look for possible files to analyze
list = dir([directory '*.zvi']);
...
for jj = 1:numel(names_list)
    ...
    if ~exist([directory 'Analytics Data\' imname 'Analytics.mat'],'file') || over_write
        ...
        %should not be a problem with the main_batch
            ...
            [colB(jj,:), colE(jj,:), colH(jj,:), colK(jj), colL(jj), colM(jj)] = piv_analytics(directory, imname, r_scale, t_scale, firstframe, user_lastframe,data_type);
