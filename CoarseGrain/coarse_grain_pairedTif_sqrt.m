function [l_c, t_c, pR, pT] = coarse_grain_pairedTif_sqrt(directory,imnames,final_frame_user,r_scale,t_scale)
...
%%%%% PARAMETERS %%%%%%
r_sizes = 2:11; %in grid points (not um or pixels), 2019/08/29 was 2 3 4 6 8 9
t_sizes = [2:2:8 10:10:100 120:20:200]; %must be even, in frames, 2019/08/29 was [2 4 10 20 40 60 80 90 100 110 120]
edge_cut = 0.75; % only keep r/R > edge_cut
options = optimset('Display','none');

% Set up a save dir if necessary
if ~exist([directory 'CoarseGrain'],'file')
    mkdir(directory,'CoarseGrain')
end
imname = [imnames{1} '_' imnames{2}];

[u,v,x,y,a,final_frame,num_pieces] = set_up_edge_piv_pairedTif(directory, imnames,final_frame_user,edge_cut,r_scale,t_scale);
...
legend(['l_c = ' num2str(l_c,'%0.0f') ' ' char(181) 'm'],'Calculated \sigma(v_\theta)')
xlabel(['Grid Size (' char(181) 'm)'],'FontSize',16)
ylabel('\sigma(v_\theta)','FontSize',16)
set(gca,'FontSize',16)
xlim([0 200])

savename = [directory 'CoarseGrain\' imname '_r fit each frame'];
...
if final_frame > max(t_sizes) %If edge detection is enough, keep going -- otherwise everything = NaN;
    ...
    x0 = [-1/pt(1) real(log(pt(2))) min(std_vth_t_f) 0];

    W = 1./std_vth_t_fs;
    
    [pT,~] = lsqnonlin(@sqrtexpweight,x0,[0 -Inf -Inf -Inf],[],options,t_scale/60*t_sizes,std_vth_t_f);
    fvalT = pT(2)*exp(-t_to_plot/pT(1)) + pT(3)./sqrt(t_to_plot/(t_scale/60))+pT(4);
    t_c = pT(1);
...
%     t_c = pT(1);
    ...
    savename = [directory 'CoarseGrain\' imname '_t fit each frame'];
    saveas(gcf,[savename '.tif'],'tif')
    saveas(gcf,[savename '.fig'],'fig')
...
%%%%%%%%%% Exponential Fitting Functions
    ...
    % Fit to a single exponential with a sqrt term and offset - 4 fit parameters
    function diff = sqrtexpweight(p,X,Y)
        fitY = p(2)*exp(-X/p(1)) + p(3)./sqrt(X/(t_scale/60)) +p(4);
        raw_diff = fitY - Y;
        diff = raw_diff.*sqrt(W);
    end
