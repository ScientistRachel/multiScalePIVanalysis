...
function  edgedat = sheet_edger_combo_v2(imname,directory,im_format,intframe,finframe,diskrad,threshm,thresh_in,threshs,savedir,frameskip,saveimpose)
...
%   images)
% v2: Same as sheet_edger, but crops off faulty microscope pixels! RML 2012/08/09 
...
% Edited by Rachel Lee, 8/26/2019
%   Cleaned up formatting and functions to match 2019a preferences for
%   displaying warnings, etc.
%   Made warning for touching the edge more flexible -- based on which side
%   is open on the first frame, new parameter: fullFrameCutOff
%   Added support for multitiff files
% Edited by Rachel Lee, 01/31/2020
%  Side cells are on is decided only by the first frame, no other frames
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
fullFrameCutOff = 2; %If more than this many pixels touch the far edge, edge detection will stop
...
% First Zeiss formats:
if strcmp(im_format,'zvi') || strcmp(im_format,'czi') || strcmp(im_format,'mvd2')
...
elseif strcmp(im_format,'multiff')
    ...
    inf = imfinfo([directory imname '.tif'],'tif');
...
if strcmp(threshm,'auto')
    ...
    for frame = intframe:finframe
        ...
        waitbar((frame-intframe)/(finframe-intframe+1),h,sprintf('%s%u%s%u%s%u%s',...
            'Processing frame: ',...
            frame,...
            '      Est time remaining: ',...
            mins,...
        ...
        if strcmp(im_format,'zvi') || strcmp(im_format,'czi') || strcmp(im_format,'mvd2')
            im = bfIms{frame,1};
        elseif strcmp(im_format,'multiff')
            im = imread([directory imname '.tif'],'tif',frame);
        else
        ...
        if frame == intframe
            ...
            elseif strcmp(lr,'right')
                sideEmpty = 2;
            elseif strcmp(lr,'top')
                sideEmpty = 3;
            elseif strcmp(lr,'bottom')
                sideEmpty = 4;
            else
                error('Monolayer Side Not Detected')
            end
...
%             [im_edge, thresh_old, lr] = open_edger(im,diskrad,'manual',thresh_old,threshs);
            [im_edge, ~, lr] = open_edger(im,diskrad,'manual',thresh_old,threshs);
            % Use the first frame to decide which side the cells will be on.
            if frame == intframe
...
%             thresh_old = thresh_out;
        ...
        %(RML 2013/07/24)
            test_thresh = find(im_edge == 1,1);
            if isempty(test_thresh)
                warning('Bad Thresholding!')
%                 [im_edge, thresh_old, lr] = open_edger(im,diskrad,'manual',0.002,threshs);
                [im_edge, ~, lr] = open_edger(im,diskrad,'manual',0.002,threshs);
                % Use the first frame to decide which side the cells will be on.
                if frame == intframe
        ...
        coords = mexdijkstra(im_edge,lr_save);%2020/01/31 this really shouldn't be necessary...need to fix filtering, but this is a stop gap for when filtering is a little rough
        ...
        a = size(coords,1);
        if(~a)
%             error('.mex dijkstra failed; matdijkstra version hasn''t been updated for vertical images yet');
            warning(['.mex dijkstra algorithm failed on frame ' num2str(frame)])
%             display(sprintf('%s%u','WARNING!: .mex dijkstra algorithm failed on frame ', frame))
            disp('   Trying .m version of dijkstra algorithm...')
            clear coords
            coords = matdijkstra(im_edge,lr_save); %2020/01/31 this really shouldn't be necessary...need to fix filtering, but this is a stop gap for when filtering is a little rough
            a = size(coords,1);
            if(a)
                disp(['   .m version succeeded in processing frame ', num2str(frame)])
            elseif(~a)
                disp(['   .m version failed in processing frame ', num2str(frame)])
                warning([' edgedat.points(:,:,' , num2str(frame), ') is empty'])
...
%                 display(sprintf('%s%u%s','   WARNING!: edgedat.points(:,:,',frame,') is empty'))
        ...
        edgedat.points(:,1,frame_count) = coords_resize(:,1);  %IF subscripted assignment mismatch on this line: Increase the size of m (too complicated of an edge).
...
%         if (a>0) || (b>0) 
%             disp(['The cells touched the edge of the image on image ' num2str(frame)])
...
% %         end
        ...
        % Check for cells leaving the frame
        sides(:,1) = sum(im_edge(:,2)); % Left
        sides(:,2) = sum(im_edge(:,end-1)); % Right
        sides(:,3) = sum(im_edge(2,:)); % Top
        sides(:,4) = sum(im_edge(end-1,:)); % Bottom
        if sides(sideEmpty) > fullFrameCutOff
            disp(['The cells touched the edge of the image on image ' num2str(frame)])
            break
        end
        
        % Write overlay frames
...
%         [im_edge, thresh_out, lr] = open_edger(im,diskrad,'manual',thresh_in,threshs);
        [im_edge, ~, lr] = open_edger(im,diskrad,'manual',thresh_in,threshs);
...
%         if (a>0) || (b>0)
%             disp(['The cells touched the edge on image ' num2str(frame)])
% %             display(sprintf('%s%u','The cells touched the edge on image ',frame))
%             break
%         end
...
% store wether or not the position was the top, left, right, or bottom of
% the monolayer
edgedat.side = lr_save;
